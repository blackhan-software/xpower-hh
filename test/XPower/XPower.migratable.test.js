...
describe("XPower Migration", async function () {
  ...
  before(async function () {
    XPowerOld = await ethers.getContractFactory("XPowerOldTest");
    XPowerNew = await ethers.getContractFactory("XPowerTest");
  });
  beforeEach(async function () {
    ...
    xpower_new = await XPowerNew.deploy([xpower_old.target], DEADLINE_NEW);
  ...
  describe("old", async function () {
    it("should mint for amount=1", async function () {
      const tx = await xpower_old.fake(addresses[0], UNIT);
      expect(tx).to.be.an("object");
      expect(await xpower_old.balanceOf(addresses[0])).to.eq(UNIT + UNIT / 2n);
    });
  });
  describe("new", async function () {
    it("should return index=0", async function () {
      const index = await xpower_new.oldIndexOf(xpower_old.target);
      expect(index).to.eq(0);
    });
    it("should mint for amount=1", async function () {
      const tx = await xpower_new.fake(addresses[0], UNIT);
      expect(tx).to.be.an("object");
      expect(await xpower_new.balanceOf(addresses[0])).to.eq(UNIT + UNIT / 2n);
    });
    it("should approve allowance old[1]", async function () {
      ...
      const [owner, spender] = [addresses[0], xpower_new.target];
      const old_increase = await xpower_old.increaseAllowance(spender, UNIT);
    ...
    it("should migrate old[0]", async function () {
      ...
      //
      {
        ...
        const tx = await xpower_old.connect(minter).fake(minter.address, 3n);
        expect(tx).to.be.an("object");
        expect(await xpower_old.balanceOf(minter.address)).to.eq(3n);
        // increase allowance for spender (i.e. XPowerNew)
        const [owner, spender] = [minter.address, xpower_new.target];
      ...
      //
      {
        ...
        expect(minter.address).to.match(/^0x/);
        const tx = await xpower_old.connect(minter).fake(minter.address, 1n);
        expect(tx).to.be.an("object");
    ...
    it("should *not* migrate old[0] (insufficient allowance)", async function () {
      const tx = await xpower_old.fake(addresses[0], 1n);
      expect(tx).to.be.an("object");
      expect(await xpower_old.balanceOf(addresses[0])).to.eq(1n);
      const new_migrate = await xpower_new.migrate(1n, [0, 0]).catch((ex) => {
    ...
    it("should *but* migrate old[0] = 1 balance", async function () {
      ...
      // increase allowance for spender (i.e. XPowerNew)
      const [owner, spender] = [addresses[0], xpower_new.target];
      const old_increase = await xpower_old.increaseAllowance(spender, 5n);
      expect(old_increase).to.be.an("object");
      const old_allowance = await xpower_old.allowance(owner, spender);
    ...
    it("should migrate old[2]", async function () {
      ...
      expect(new_allowance).to.eq(0n);
      // migrate amount from old[owner] to new[owner]
      const new_migrate = await xpower_new.connect(minter).migrate(1n, [0, 0]);
      expect(new_migrate).to.be.an("object");
    ...
    it("should *not* migrate old[2] (migration sealed)", async function () {
      ...
      xpower_new = await XPowerNew.deploy([xpower_old.target], DEADLINE_NEW);
      expect(xpower_new.target).to.match(/^0x[0-9a-f]/i);
      // increase allowance for spender (i.e. XPowerNew)
      const [owner, spender] = [minter.address, xpower_new.target];
      const old_increase = await xpower_old
        .connect(minter)
    ...
    it("should *not* migrate old[2] (deadline passed)", async function () {
      ...
      // migrate amount from old[owner] to new[owner]
      const new_migrate_1 = await xpower_new
        .connect(minter)
        .migrate(1n, [0, 0]);
      ...
      await network.provider.send("evm_increaseTime", [604_800]);
      // migrate amount from old[owner] to new[owner]
      const new_migrate_2 = await xpower_new
        .connect(minter)
        .migrate(1n, [0, 0]);
