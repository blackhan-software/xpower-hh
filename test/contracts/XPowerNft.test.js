...
describe("XPowerNft", async function () {
  ...
  describe("mint-batch", async function () {
    ...
    it("should *not* mint NFTs (non-ternary leve)", async function () {
      expect(
        await nftMintBatch(15, 2).catch((ex) => {
          const m = ex.message.match(/non-ternary level/);
          if (m === null) console.debug(ex);
          expect(m).to.be.not.null;
        })
  ...
  describe("burn-batch", async function () {
    ...
    it("should *not* burn NFTs (irredeemable issue)", async function () {
      ...
      await nftMint(95);
      expect(
        await nftBurnBatch(4, 3)().catch((ex) => {
          const m = ex.message.match(/irredeemable issue/);
          if (m === null) console.debug(ex);
          expect(m).to.be.not.null;
        })
    ...
    it("should *not* burn NFTs (non-ternary level)", async function () {
      expect(
        await nftBurnBatch(15, 2)().catch((ex) => {
          const m = ex.message.match(/non-ternary level/);
          if (m === null) console.debug(ex);
          expect(m).to.be.not.null;
        })
