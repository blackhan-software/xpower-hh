...
describe("MoeTreasury", async function () {
  ...
  describe("claimFor", async function () {
    it("should return 601'000 [QRSH] in 36 months", async function () {
      ...
      expect(await mt.rewardOf(account, nft_id)).to.eq(30_000);
      expect(await mt.totalRewardOf(nft_id)).to.eq(30_000);
      expect(await mt.claimedFor(account, nft_id)).to.eq(30_000);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(30_000);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 30_000);
      // wait for +6 months: 1st year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(90_015);
      expect(await mt.totalRewardOf(nft_id)).to.eq(90_015);
      expect(await mt.claimedFor(account, nft_id)).to.eq(90_015);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(90_015);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 90_015);
      // wait for +6 months: 2nd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(150_050);
      expect(await mt.totalRewardOf(nft_id)).to.eq(150_050);
      expect(await mt.claimedFor(account, nft_id)).to.eq(150_050);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(150_050);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 150_050);
      // wait for +6 months: 3rd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(210_105);
      expect(await mt.totalRewardOf(nft_id)).to.eq(210_105);
      expect(await mt.claimedFor(account, nft_id)).to.eq(210_105);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(210_105);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 210_105);
      // wait for +6 months: 4th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(240160);
      expect(await mt.totalRewardOf(nft_id)).to.eq(240160);
      expect(await mt.claimedFor(account, nft_id)).to.eq(240160);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(240160);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 240160);
      // wait for +6 months: 5th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(270_180);
      expect(await mt.totalRewardOf(nft_id)).to.eq(270_180);
      expect(await mt.claimedFor(account, nft_id)).to.eq(270_180);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(270_180);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 270180);
      // wait for +6 months: 5th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(330_275);
      expect(await mt.totalRewardOf(nft_id)).to.eq(330_275);
      expect(await mt.claimedFor(account, nft_id)).to.eq(330_275);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(330_275);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 330275);
      // wait for +6 months: 6th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(390_390);
      expect(await mt.totalRewardOf(nft_id)).to.eq(390_390);
      expect(await mt.claimedFor(account, nft_id)).to.eq(390_390);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(390_390);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 390_390);
      // wait for +6 months: 7th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(450_525);
      expect(await mt.totalRewardOf(nft_id)).to.eq(450_525);
      expect(await mt.claimedFor(account, nft_id)).to.eq(450_525);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(450_525);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 450_525);
      // wait for +6 months: 8th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(510_680);
      expect(await mt.totalRewardOf(nft_id)).to.eq(510_680);
      expect(await mt.claimedFor(account, nft_id)).to.eq(510_680);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(510_680);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 510_680);
      // wait for +6 months: 9th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(570_855);
      expect(await mt.totalRewardOf(nft_id)).to.eq(570_855);
      expect(await mt.claimedFor(account, nft_id)).to.eq(570_855);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(570_855);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(601_000 - 570_855);
      // wait for +6 months: 10th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
