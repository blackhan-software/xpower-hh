...
describe("MoeTreasury", async function () {
  ...
  before(async function () {
    await xodin.transfer(mty.address, 210_000n * UNIT);
  });
  describe("moeBalance", async function () {
    it("should return 210'000 [ODIN]", async function () {
      ...
      expect(await mty.moeBalanceOf(moe_index)).to.eq(210_000n * UNIT);
    });
  });
  describe("claimFor", async function () {
    it("should set average APR to 2.00[%]", async function () {
      await mty.grantRole(mty.APR_ROLE(), addresses[0]);
      await mty.setAPR(3202106, [0, 3, 2_000_000]);
    });
    it("should return 210'000 [ODIN] in 120 months", async function () {
      ...
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT);
      ...
      expect(await mty.rewardOf(account, nft_id)).to.eq(10_000n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(10_000n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 10_000n * UNIT);
      // wait for +6 months: 1st year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(20_100n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(20_100n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 20_100n * UNIT);
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(30_150n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(30_150n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 30_150n * UNIT);
      // wait for +6 months: 2nd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(40_400n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(40_400n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 40_400n * UNIT);
      // wait for +6 months: 3rd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(50_500n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(50_500n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 50_500n * UNIT);
      // wait for +6 months: 3rd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(60_900n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(60_900n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 60_900n * UNIT);
      // wait for +6 months: 4th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(71_050n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(71_050n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 71_050n * UNIT);
      // wait for +6 months: 4th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(81_600n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(81_600n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 81_600n * UNIT);
      // wait for +6 months: 5th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(91_800n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(91_800n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(210_000n * UNIT - 91_800n * UNIT);
      // wait for +6 months: 5th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(102_500n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(102_500n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 102_500n * UNIT
      );
      // wait for +6 months: 6th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(112_750n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(112_750n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 112_750n * UNIT
      );
      // wait for +6 months: 6th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(123_600n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(123_600n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 123_600n * UNIT
      );
      // wait for +6 months: 7th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(133_900n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(133_900n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 133_900n * UNIT
      );
      // wait for +6 months: 7th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(144_900n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(144_900n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 144_900n * UNIT
      );
      // wait for +6 months: 8th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(155_250n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(155_250n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 155_250n * UNIT
      );
      // wait for +6 months: 8th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(166_400n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(166_400n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 166_400n * UNIT
      );
      // wait for +6 months: 9th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(176_800n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(176_800n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 176_800n * UNIT
      );
      // wait for +6 months: 9th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(188_100n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(188_100n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 188_100n * UNIT
      );
      // wait for +6 months: 10th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(198_550n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(198_550n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 198_550n * UNIT
      );
      // wait for +6 months: 10th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(210_000n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(210_000n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
      expect(await mty.moeBalanceOf(2)).to.eq(
        210_000n * UNIT - 210_000n * UNIT
      );
      // wait for +12 months: 11th year (empty treasury)
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 1.0]);
      expect(await mty.claimFor(account, nft_id)).to.be.an("object");
      expect(await mty.rewardOf(account, nft_id)).to.eq(232_100n * UNIT);
      expect(await mty.claimedFor(account, nft_id)).to.eq(232_100n * UNIT);
      expect(await mty.claimableFor(account, nft_id)).to.eq(0);
