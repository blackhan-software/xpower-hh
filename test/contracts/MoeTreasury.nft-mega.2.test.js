...
describe("MoeTreasury", async function () {
  ...
  describe("claimFor", async function () {
    it("should return 601'000 [ODIN] in 36 months", async function () {
      ...
      expect(await mt.rewardOf(account, nft_id)).to.eq(10_050n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(10_050n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(10_050n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(10_050n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 10_050n * UNUM);
      // wait for +6 months: 1st year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(30_300n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(30_300n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(30_300n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(30_300n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 30_300n * UNUM);
      // wait for +6 months: 2nd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(50_750n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(50_750n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(50_750n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(50_750n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 50_750n * UNUM);
      // wait for +6 months: 3rd year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(71_400n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(71_400n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(71_400n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(71_400n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 71_400n * UNUM);
      // wait for +6 months: 4th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(92_250n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(92_250n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(92_250n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(92_250n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 92_250n * UNUM);
      // wait for +6 months: 5th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(113_300n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(113_300n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(113_300n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(113_300n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 113_300n * UNUM);
      // wait for +6 months: 6th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(134_550n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(134_550n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(134_550n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(134_550n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 134_550n * UNUM);
      // wait for +6 months: 7th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(156_000n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(156_000n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(156_000n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(156_000n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 156_000n * UNUM);
      // wait for +6 months: 8th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(177_650n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(177_650n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(177_650n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(177_650n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 177_650n * UNUM);
      // wait for +6 months: 9th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      ...
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
      expect(await mt.rewardOf(account, nft_id)).to.eq(199_500n * UNUM);
      expect(await mt.totalRewardOf(nft_id)).to.eq(199_500n * UNUM);
      expect(await mt.claimedFor(account, nft_id)).to.eq(199_500n * UNUM);
      expect(await mt.totalClaimedFor(nft_id)).to.eq(199_500n * UNUM);
      expect(await mt.claimableFor(account, nft_id)).to.eq(0);
      expect(await mt.totalClaimableFor(nft_id)).to.eq(0);
      expect(await mt.balance()).to.eq(210_000n * UNUM - 199_500n * UNUM);
      // wait for +6 months: 10th year
      await network.provider.send("evm_increaseTime", [365.25 * DAYS * 0.5]);
      expect(await mt.claimFor(account, nft_id)).to.be.an("object");
