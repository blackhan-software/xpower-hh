...
function ppt_bases(
  token,
  versions = ["V4a", "V5a", "V5b", "V5c", "V6a", "V6b", "V6c", "V7a", "V7b"]
) {
...
async function main() {
  ...
  assert(thor_ppt_base.length === 9);
  const loki_ppt_base = ppt_bases("LOKI");
  assert(loki_ppt_base.length === 9);
  const odin_ppt_base = ppt_bases("ODIN");
  assert(odin_ppt_base.length === 9);
  ...
  console.log(`XPOW_PPT_V7c=${xpow.ppt.address}`);
  //
  // verify contract(s):
  //
