...
async function main() {
  ...
  const thor_sov_link = process.env.THOR_SOV_V7c;
  assert(thor_sov_link, "missing THOR_SOV_V7c");
  const loki_sov_link = process.env.LOKI_SOV_V7c;
  assert(loki_sov_link, "missing LOKI_SOV_V7c");
  const odin_sov_link = process.env.ODIN_SOV_V7c;
  assert(odin_sov_link, "missing ODIN_SOV_V7c");
  ...
  console.log(`XPOW_MTY_V7c=${xpow_treasury.address}`);
  //
  // verify contract(s):
  //
  await verify(
