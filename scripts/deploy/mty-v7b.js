...
async function main() {
  ...
  const thor_sov_link = process.env.THOR_SOV_V7b;
  assert(thor_sov_link, "missing THOR_SOV_V7b");
  const loki_sov_link = process.env.LOKI_SOV_V7b;
  assert(loki_sov_link, "missing LOKI_SOV_V7b");
  const odin_sov_link = process.env.ODIN_SOV_V7b;
  assert(odin_sov_link, "missing ODIN_SOV_V7b");
  // addresses XPowerPpt[New]
  const xpow_ppt_link = process.env.XPOW_PPT_V7b;
  assert(xpow_ppt_link, "missing XPOW_PPT_V7b");
  //
  ...
  console.log(`XPOW_MTY_V7b=${xpow_treasury.address}`);
  //
  // verify contract(s):
  //
  await verify(
