...
function moe_bases(
  token,
  versions = ["V2a", "V3a", "V4a", "V5a", "V5b", "V5c", "V6a", "V6b"]
) {
...
async function main() {
  ...
  assert(thor_moe_base.length === 8);
  const loki_moe_base = moe_bases("LOKI");
  assert(loki_moe_base.length === 8);
  const odin_moe_base = moe_bases("ODIN");
  assert(odin_moe_base.length === 8);
  // migration:
  ...
  console.log(`ODIN_MOE_V6c=${odin.moe.address}`);
  //
  // verify contract(s):
  //
