...
async function main() {
  ...
  const xpow_nft_link = process.env.XPOW_NFT_V7b;
  assert(xpow_nft_link, "missing XPOW_NFT_V7b");
  ...
  const xpow_mty_link = process.env.XPOW_MTY_V7b;
  assert(xpow_mty_link, "missing XPOW_MTY_V7b");
  //
  ...
  console.log(`XPOW_PTY_V7b=${xpow.nty.address}`);
  //
  // verify contract(s):
  //
  await verify(
