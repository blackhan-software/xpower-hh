...
async function main() {
  ...
  const moe_link = process.env.XPOW_MOE_V9b;
  assert(moe_link, "missing XPOW_MOE_V9b");
  // addresses APower[New]
  const sov_link = process.env.XPOW_SOV_V9b;
  assert(sov_link, "missing XPOW_SOV_V9b");
  // addresses XPowerPpt[New]
  const ppt_link = process.env.XPOW_PPT_V9b;
  assert(ppt_link, "missing XPOW_PPT_V9b");
  //
  ...
  console.log(`XPOW_MTY_V9b=${mty.target}`);
  //
  // verify contract(s):
  //
