...
async function main() {
  ...
  const nft_link = process.env.XPOW_NFT_V5b;
  assert(nft_link, "missing XPOW_NFT_V5b");
  // addresses XPowerPpt[New]
  const ppt_link = process.env.XPOW_PPT_V5b;
  assert(ppt_link, "missing XPOW_PPT_V5b");
  // addresses MoeTreasury[New]
  const mty_link = process.env.XPOW_MTY_V5b;
  assert(mty_link, "missing XPOW_MTY_V5b");
  //
