 ...
 */
function sov_bases(token, versions = ["V5a", "V5b", "V5c"]) {
...
async function main() {
  ...
  assert(thor_sov_base.length === 3);
  const loki_sov_base = sov_bases("LOKI");
  assert(loki_sov_base.length === 3);
  const odin_sov_base = sov_bases("ODIN");
  assert(odin_sov_base.length === 3);
  ...
  console.log(`THOR_SOV_V6a=${thor.sov.address}`);
  //
  ...
  console.log(`LOKI_SOV_V6a=${loki.sov.address}`);
  //
  ...
  console.log(`ODIN_SOV_V6a=${odin.sov.address}`);
  //
  // verify contract(s):
  //
