...
async function main() {
  ...
  assert(thor_moe_base.length === 11);
  const loki_moe_base = moe_bases("LOKI");
  assert(loki_moe_base.length === 11);
  const odin_moe_base = moe_bases("ODIN");
  assert(odin_moe_base.length === 11);
  // migration:
  ...
  console.log(`THOR_MOE_V7c=${thor.moe.address}`);
  //
  ...
  console.log(`LOKI_MOE_V7c=${loki.moe.address}`);
  //
  ...
  console.log(`ODIN_MOE_V7c=${odin.moe.address}`);
  //
  // verify contract(s):
  //
