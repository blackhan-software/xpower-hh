...
function moe_bases(
  token,
  versions = ["V2a", "V3a", "V4a", "V5a", "V5b", "V5c", "V6a"]
) {
...
async function main() {
  ...
  assert(thor_moe_base.length === 7);
  const loki_moe_base = moe_bases("LOKI");
  assert(loki_moe_base.length === 7);
  const odin_moe_base = moe_bases("ODIN");
  assert(odin_moe_base.length === 7);
  // migration:
  ...
  console.log(`THOR_MOE_V6b=${thor.moe.address}`);
  //
  // deploy XPower[New]
  //
  ...
  console.log(`LOKI_MOE_V6b=${loki.moe.address}`);
  //
  // deploy XPower[New]
  //
  ...
  console.log(`ODIN_MOE_V6b=${odin.moe.address}`);
  //
  // verify contract(s):
  //
