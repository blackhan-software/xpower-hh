...
async function main() {
  ...
  assert(thor_sov_base.length === 6);
  const loki_sov_base = sov_bases("LOKI");
  assert(loki_sov_base.length === 6);
  const odin_sov_base = sov_bases("ODIN");
  assert(odin_sov_base.length === 6);
  ...
  console.log(`THOR_SOV_V7a=${thor.sov.address}`);
  //
  ...
  console.log(`LOKI_SOV_V7a=${loki.sov.address}`);
  //
  ...
  console.log(`ODIN_SOV_V7a=${odin.sov.address}`);
  //
  // verify contract(s):
  //
