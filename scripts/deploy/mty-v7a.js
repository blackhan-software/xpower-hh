...
async function main() {
  ...
  const moe_link = process.env.XPOW_MOE_V7a;
  assert(moe_link, "missing XPOW_MOE_V7a");
  // addresses APower[New]
  const sov_link = process.env.XPOW_SOV_V7a;
  assert(sov_link, "missing XPOW_SOV_V7a");
  ...
  console.log(`XPOW_MTY_V7a=${mty.target}`);
  //
  // verify contract(s):
  //
