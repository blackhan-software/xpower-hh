...
async function main() {
  ...
  const nft_link = process.env.XPOW_NFT_V9c;
  assert(nft_link, "missing XPOW_NFT_V9c");
  // addresses XPowerPpt[New]
  const ppt_link = process.env.XPOW_PPT_V9c;
  assert(ppt_link, "missing XPOW_PPT_V9c");
  // addresses MoeTreasury[New]
  const mty_link = process.env.XPOW_MTY_V9c;
  assert(mty_link, "missing XPOW_MTY_V9c");
  //
  ...
  console.log(`XPOW_PTY_V9c=${nty.target}`);
  //
  // verify contract(s):
  //
