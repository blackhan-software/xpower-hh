...
async function main() {
  ...
  const thor_sov_link = process.env.THOR_SOV_V6c;
  assert(thor_sov_link, "missing THOR_SOV_V6c");
  const loki_sov_link = process.env.LOKI_SOV_V6c;
  assert(loki_sov_link, "missing LOKI_SOV_V6c");
  const odin_sov_link = process.env.ODIN_SOV_V6c;
  assert(odin_sov_link, "missing ODIN_SOV_V6c");
  ...
  console.log(`XPOW_MTY_V6c=${xpow_treasury.address}`);
  //
  // verify contract(s):
  //
  await verify(
