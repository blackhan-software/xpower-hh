...
async function main() {
  ...
  const moe_link = process.env.XPOW_MOE_V6c;
  assert(moe_link, "missing XPOW_MOE_V6c");
  // addresses APower[New]
  const sov_link = process.env.XPOW_SOV_V6c;
  assert(sov_link, "missing XPOW_SOV_V6c");
  ...
  console.log(`XPOW_MTY_V6c=${mty.target}`);
  //
  // verify contract(s):
  //
