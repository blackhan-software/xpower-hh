...
async function main() {
  ...
  const moe_link = process.env.XPOW_MOE_V9c;
  assert(moe_link, "missing XPOW_MOE_V9c");
  // addresses APower[New]
  const sov_link = process.env.XPOW_SOV_V9c;
  assert(sov_link, "missing XPOW_SOV_V9c");
  ...
  console.log(`XPOW_MTY_V9c=${mty.target}`);
  //
  // verify contract(s):
  //
