 ...
 */
function sov_bases(token, versions = ["V5a", "V5b", "V5c", "V6a", "V6b"]) {
...
async function main() {
  ...
  assert(thor_sov_base.length === 5);
  const loki_sov_base = sov_bases("LOKI");
  assert(loki_sov_base.length === 5);
  const odin_sov_base = sov_bases("ODIN");
  assert(odin_sov_base.length === 5);
  ...
  console.log(`THOR_SOV_V6c=${thor.sov.address}`);
  //
  // deploy APower[New]
  //
  ...
  console.log(`LOKI_SOV_V6c=${loki.sov.address}`);
  //
  // deploy APower[New]
  //
  ...
  console.log(`ODIN_SOV_V6c=${odin.sov.address}`);
  //
  // verify contract(s):
  //
