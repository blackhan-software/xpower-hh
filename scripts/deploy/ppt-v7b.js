...
function ppt_bases(
  token,
  versions = ["V4a", "V5a", "V5b", "V5c", "V6a", "V6b", "V6c", "V7a"]
) {
...
async function main() {
  ...
  assert(thor_ppt_base.length === 8);
  const loki_ppt_base = ppt_bases("LOKI");
  assert(loki_ppt_base.length === 8);
  const odin_ppt_base = ppt_bases("ODIN");
  assert(odin_ppt_base.length === 8);
  ...
  console.log(`XPOW_PPT_V7b=${xpow.ppt.address}`);
  //
  // verify contract(s):
  //
