...
async function main() {
  ...
  const xpow_nft_link = process.env.XPOW_NFT_V5a;
  assert(xpow_nft_link, "missing XPOW_NFT_V5a");
  // addresses XPowerPpt[New]
  const xpow_ppt_link = process.env.XPOW_PPT_V5a;
  assert(xpow_ppt_link, "missing XPOW_PPT_V5a");
  // addresses MoeTreasury[New]
  const xpow_mty_link = process.env.XPOW_MTY_V5a;
  assert(xpow_mty_link, "missing XPOW_MTY_V5a");
  //
  ...
  console.log(`XPOW_PTY_V5a=${nty.target}`);
  //
  // verify contract(s):
  //
